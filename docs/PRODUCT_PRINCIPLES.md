...
**Terminology / Thuật ngữ (product docs):** **Mc** (*Mờ Cê*) = one **Merchant** tenant (`mid`); **Mcs** (*Mờ C S*) = **Merchants** host (API + web entry). See [Wire ≠ MoMo](wire-vs-momo.md).
...
**EN:** **Mcs tenants (Mc, and future banks-as-`mid`)** share the **same wire protocol and idempotency rules**; they differ by **policy, quotas, and secrets**. The platform sells **reliable rails and tenancy**, not one-off bespoke pipes per partner.

**VI:** **Tenant trên Mcs (Mc, và sau này bank như `mid`)** dùng **chung wire và luật idempotency**; khác nhau ở **policy, quota, secret**. Nền tảng bán **đường ray tin cậy và đa tenant**, không phải ống tùy hứng từng đối tác.

**Shopify-like:** one **Mcs** codebase and one **Wire** app serve **many Mc** shops (`mid`, slug, domain)—see [Multi-tenant Mcs](multi-tenant-mcs.md).

**Kiểu Shopify:** một codebase **Mcs**, một app **Wire**, phục vụ **nhiều Mc** (`mid`, slug, domain)—xem [Đa tenant Mcs](multi-tenant-mcs.md).
...
**EN:** **`extraData` is opaque** on the hot path unless a command requires structured parsing (e.g. confirm). It may be **empty**. Size is **capped** at ingress. Each **Mc** owns the meaning inside the bag; the platform guarantees **integrity** (MAC covers `command` through `extraData`) and **limits**, not every business schema.

**VI:** **`extraData` là opaque** trên hot path trừ khi lệnh bắt parse (ví dụ confirm). Có thể **rỗng**. Kích thước **bị giới hạn** lúc vào cổng. **Mc** định nghĩa nội dung “túi”; nền tảng đảm bảo **toàn vẹn** (MAC phủ `command`…`extraData`) và **giới hạn**, không ôm mọi schema nghiệp vụ.
...
**Super App** là **vỏ host**: trang chủ và điều hướng, **sạp Mc** (§3), **discovery** danh mục / hồ sơ, và **ô tìm** trong app (xem [ADR 002](adr/002-search-boundary-meilisearch.md)). Super App nắm **UX lớp vỏ và bàn giao người dùng sang mini-app** (ví dụ SavingApp, sạp đối tác); **không** nắm số dư, hold, hay trạng thái thanh toán—các thứ đó vẫn thuộc **luồng ví** (§1, §4). Index tìm kiếm / duyệt **có thể trễ**; **không** dùng làm chứng cứ tiền hay đã settle.
...
**EN:** Use it for onboarding, partner decks, and **copy review**: if a sentence contradicts a principle here, fix the sentence or update this page with a dated rationale. For **Wire superapp vs MoMo-style redirect**, see [Wire ≠ MoMo](wire-vs-momo.md).

**VI:** Dùng cho onboarding, deck đối tác, và **duyệt copy**: nếu câu nào mâu thuẫn nguyên tắc ở đây, sửa câu đó hoặc cập nhật trang này kèm lý do và thời điểm. Với **siêu app Wire vs redirect kiểu MoMo**, xem [Wire ≠ MoMo](wire-vs-momo.md).
