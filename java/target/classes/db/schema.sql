...
CREATE TABLE IF NOT EXISTS sav_trans (
  ...
  pending_id        BIGINT      REFERENCES sav_trans(id), -- non-null for POSTED/VOIDED settlement rows
  idempotency_key   BIGINT      UNIQUE,             -- client-supplied; server deduplicates
  ref_mid           BIGINT,                         -- link to wallet mid
  ref_request_id    BIGINT,                         -- link to wallet request_id
  linked_batch_id   BIGINT,                         -- groups atomic-batch transfers (interest accrual)
  memo              VARCHAR(256),
  created_at        TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
